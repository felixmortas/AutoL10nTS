...
export class L10nProcessor {
    ...
    async process() {
        ...
        try {
            ...
            console.warn(`[WARNING] Target ARB file not found, initialized to empty object: ${targetArbPath}`);
        }
        ...
        for (const filePathStr of files) {
            ...
            const finalResponse = await this.llm.process(flutterContent, arbContent, langTag);
        ...
        for (const arbFile of otherArbFiles) {
            ...
            const translated = await this.llm.amendArb(JSON.stringify(fullArbLines, null, 2), lang);
            let existing = "{}";
            try {
